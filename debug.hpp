...
#define TIMER_STOP(timer, message) {\
    ...
    auto t = std::chrono::duration_cast<std::chrono::milliseconds>(now - logger_state.beginning).count();\
...
#define TIMER_STOP_COND(cond, timer, message) if(cond) TIMER_STOP(timer, message)
#else
...
#define MESSAGE(level, message) {\
    ...
    auto now = std::chrono::steady_clock::now();\
    if (logger_state.logger != nullptr)\
        (*logger_state.logger) << "[" << level << " " << std::chrono::duration_cast<std::chrono::milliseconds>(now - logger_state.beginning).count()\
